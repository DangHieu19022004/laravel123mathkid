...
class MeasurementGameController extends AbstractGroupGameController
{
    ...
    // 11. Đo Dung Tích Game
    public function volumeMeasurementGame()
    {
        $question = $this->generateCompareVolumeQuestion();
        return view('games.lop4.dailuongvadoluong.volume_measurement', compact('question'));
    }
    ...
    public function checkVolumeMeasurementAnswer(Request $request)
    {
        ...
        $answer = $request->input('answer');
        $correct = ($answer == $question['actual_volume']);
        if ($correct && $level < 5) {
        ...
        return response()->json([
            'correct' => $correct,
            'next_level' => $correct && $level < 5,
            'actual_volume' => $question['actual_volume']
        ]);
    }

    public function resetVolumeMeasurementGame()
    {
        session()->forget('volume_measurement_level');
        return redirect()->route('games.lop4.dailuongvadoluong.volume_measurement');
    }
    ...
    public function perimeterCalculationGame()
    {
        ...
        return view('games.lop4.dailuongvadoluong.perimeter_calculation', compact('question'));
    }
    ...
    private function calculatePerimeter($shape, $sides)
    {
        switch ($shape) {
            case 'hình vuông':
                return $sides[0] * 4;
    ...
    public function resetPerimeterCalculationGame()
    {
        session()->forget('perimeter_calculation_level');
        return redirect()->route('games.lop4.dailuongvadoluong.perimeter_calculation');
    }

    // 13. Tính Diện Tích Game
    public function areaCalculationGame()
    {
        ...
        return view('games.lop4.dailuongvadoluong.area_calculation', compact('question'));
    }
    ...
    private function generateAreaCalculationQuestion($level)
    {
        $questions = [
            ...
            2 => [
                'shape' => 'hình chữ nhật',
                'dimensions' => ['chiều dài' => 6, 'chiều rộng' => 4],
                'unit' => 'cm²'
            ],
            3 => [
                'shape' => 'tam giác',
                'dimensions' => ['đáy' => 6, 'chiều cao' => 4],
                'unit' => 'cm²'
            ],
            4 => [
                'shape' => 'hình thang',
                'dimensions' => ['đáy lớn' => 8, 'đáy nhỏ' => 6, 'chiều cao' => 4],
                'unit' => 'cm²'
            ],
            5 => [
                'shape' => 'hình bình hành',
                'dimensions' => ['đáy' => 7, 'chiều cao' => 5],
                'unit' => 'cm²'
            ]
        ];
    ...
    private function calculateArea($shape, $dimensions)
    {
        switch ($shape) {
            case 'hình vuông':
                return pow($dimensions['cạnh'], 2);
            case 'hình chữ nhật':
                return $dimensions['chiều dài'] * $dimensions['chiều rộng'];
            case 'tam giác':
                return ($dimensions['đáy'] * $dimensions['chiều cao']) / 2;
            case 'hình thang':
                return (($dimensions['đáy lớn'] + $dimensions['đáy nhỏ']) * $dimensions['chiều cao']) / 2;
            case 'hình bình hành':
                return $dimensions['đáy'] * $dimensions['chiều cao'];
            default:
                return 0;
        }
    }

    public function resetAreaCalculationGame()
    {
        session()->forget('area_calculation_level');
        return redirect()->route('games.lop4.dailuongvadoluong.area_calculation');
    }
    ...
    public function angleMeasurementGame()
    {
        ...
        return view('games.lop4.dailuongvadoluong.angle_measurement', compact('question'));
    }
    ...
    public function checkAngleMeasurementAnswer(Request $request)
    {
        ...
        return response()->json([
            'correct' => $correct,
            'next_level' => $correct && $level < 5,
            'actual_angle' => $question['actual_angle']
        ]);
    }

    public function resetAngleMeasurementGame()
    {
        session()->forget('angle_measurement_level');
        return redirect()->route('games.lop4.dailuongvadoluong.angle_measurement');
    }
    ...
    private function calculateTimeDifference($start, $end, $nextDay = false)
    {
    ...
    public function resetAdvancedTimeGame()
    {
